...
ifeq ($(MW_VERSION), v1)
	...
	ifeq ($(CHIP), CV186X)
		...
		MW_AUDIO_LIBS += -lcvi_dnvqe -lcvi_ssp2
...
$(info CFLAGS: $(CFLAGS))
$(info SAMPLE_VI_LIBS: $(SAMPLE_VI_LIBS))
...
TARGETS_GKING := $(shell find . -type f -name 'g_main*.c' -exec basename {} .c ';')
TARGETS_GKING += $(shell find . -type f -name 'g_main*.cpp' -exec basename {} .cpp ';')
...
$(PWD)/g_utils/%.o: $(PWD)/g_utils/%.c
	$(CC) $(CFLAGS) -o $@ -c $<

$(PWD)/g_utils/%.o: $(PWD)/g_utils/%.cpp
	$(CXX) $(CFLAGS) -o $@ -c $<

$(PWD)/g_utils/Gking_milkvduo_util.so : $(wildcard $(PWD)/g_utils/*.cpp)
	$(CXX) -fPID -shared $@ -o libGMU.so
...
g_main%: $(PWD)/g_main%.o \
			 $(SDK_ROOT_PATH)/sample/utils/sample_utils.o \
			 $(SDK_ROOT_PATH)/sample/utils/middleware_utils.o \
			 $(PWD)/g_utils/g_util_camera.o \
			 $(PWD)/g_utils/g_util_gpio.o \
			 $(PWD)/g_utils/g_modify.o \
			 $(SAMPLE_COMMON_FILE)
	$(info $(CXX) $(CFLAGS) $(SAMPLE_VI_LIBS) -o $@ $^)
	$(CXX) $(CFLAGS) $(SAMPLE_VI_LIBS) -o $@ $^
