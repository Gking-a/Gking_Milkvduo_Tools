...
void helpmethodtogp(int gp){
    ...
    {
        if(gp==hardcode_milkv_gp_pwm[i*2]){
            char pwm=hardcode_milkv_gp_pwm[i*2];
            char cmd[100];
            snprintf(cmd,100,"duo-pinmux -w GP%d/GP%d",gp,pwm);
            system(cmd);
            return;
        }
    }
...
void L298NController::setspeed(int chan,int speed){
    ...
    if(speed>=1000||speed<=-1000){
        ...
        wiringOpr(this->port.chan[chan][0],1);
    }else if(speed==0){
        standardPWM(pwm_pin,0);
        wiringOpr(this->port.chan[chan][2],0);
        wiringOpr(this->port.chan[chan][0],0);
    }else if (speed>0)
    {
        printf("into speed 0,1000");
        int pwm_pin=port.chan[chan][0];
        ...
        standardPWM(pwm_pin,-speed);
        wiringXPWMEnable(port.chan[chan][0],1);
